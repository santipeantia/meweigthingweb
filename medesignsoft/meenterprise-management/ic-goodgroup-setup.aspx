<%@ Page Title="" Language="C#" MasterPageFile="~/medesignsoft.Master" AutoEventWireup="true" CodeBehind="ic-goodgroup-setup.aspx.cs" Inherits="medesignsoft.meenterprise_management.ic_goodgroup_setup" %>
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    <section class="content-header">
        ...
        <script>
            $(document).ready(function () {
                ...
                btnaddnew.click(function () {
                    window.location.href = "ic-goodgroup-edit.aspx?opt=optic&mod=new";
                });
                ...
                function reloaddata() {
                    $.ajax({
                        ...
                        beforeSend: function () {
                            $('#loader').show();
                            $("#tblgoodgrouplist tr td").remove();

                        },
                        success: function (data) {
                            var table;
                            table = $('#tblgoodgrouplist').DataTable();
                            table.clear();

                            if (data != '') {
                                $.each(data, function (i, item) {
                                    table.row.add([data[i].GoodGroupID, data[i].GoodGroupCode, data[i].GoodGroupDesc
                            ...
                            $('#tblgoodgrouplist td:nth-of-type(4)').addClass('column_hover');
                            $('#tblgoodgrouplist td:nth-of-type(5)').addClass('column_hover');

                            $('#tblgoodgrouplist td').click(function () {
                                rIndex = this.parentElement.rowIndex;
                                cIndex = this.cellIndex;
                                console.log('row : ' + rIndex + ' cell: ' + cIndex)

                                if (rIndex != 0 & cIndex == 4) {
                                    var gid = $("#tblgoodgrouplist").find('tr:eq(' + rIndex + ')').find('td:eq(0)');
                                    ...
                                    window.location.href = "ic-goodgroup-edit.aspx?opt=optic&mod=edit&gid=" + gid.text();
                                }

                                if (rIndex != 0 & cIndex == 5) {
                                    var gid = $("#tblgoodgrouplist").find('tr:eq(' + rIndex + ')').find('td:eq(0)');
                                    console.log(gid.text());
                                    window.location.href = "ic-goodgroup-edit.aspx?opt=optic&mod=del&gid=" + gid.text();
                                }
        ...
        <h1>Good Group Setup <%--step 1 check pages content name--%>
                ...
                <div class="">
                    <div class="box box-solid">
                        ...
                        <div class="box-body">
                            ...
                            <table id="tblgoodgrouplist" class="table table-striped table-bordered table-hover table-condensed" style="width: 100%">
                                <thead>
                                    <tr>
                                        <th>ID</th>
                                        <th>CODE</th>
