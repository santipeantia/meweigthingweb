<%@ Page Title="" Language="C#" MasterPageFile="~/medesignsoft.Master" AutoEventWireup="true" CodeBehind="gm-section-setup.aspx.cs" Inherits="medesignsoft.meenterprise_management.gm_section_setup" %>
        ...
        <script>
            $(document).ready(function () {
                ...
                btnaddnew.click(function () {
                     window.location.href = "gm-section-edit.aspx?opt=optgen&mod=new";
                });
                ...
                function reloaddata() {
                    $.ajax({
                        ...
                        beforeSend: function () {
                            $('#loadersection').show();
                            $("#tblsectionlist tr td").remove();

                        },
                        success: function (data) {
                            var table;
                            table = $('#tblsectionlist').DataTable();
                            table.clear();

                            if (data != '') {
                                $.each(data, function (i, item) {
                                    table.row.add([data[i].Gid, data[i].imSectionID, data[i].SectionDesc, data[i].SectionDesc2, data[i].Active, data[i].ActiveName, data[i].EffectDate, data[i].ExpireDate, data[i].edit, data[i].trash]);
                                });
                            }
                            table.draw();
                            $('#loadersection').hide();
                            ...
                            $('#tblsectionlist td').click(function () {
                                rIndex = this.parentElement.rowIndex;
                                cIndex = this.cellIndex;
                                console.log('row : ' + rIndex + ' cell: ' + cIndex)

                                if (rIndex != 0 & cIndex == 8) {
                                    var gid = $("#tblsectionlist").find('tr:eq(' + rIndex + ')').find('td:eq(0)');
                                    console.log(gid.text());

                                    window.location.href = "gm-section-edit.aspx?opt=optgen&mod=edit&gid="+gid.text();
                                ...
                                if (rIndex != 0 & cIndex == 9) {
                                    ...
                                    window.location.href = "gm-section-edit.aspx?opt=optgen&mod=del&gid=" + gid.text();
                                }
                ...
                <div class="">
                    <div class="box box-solid">
                        <div class="box-header with-border">
                            ...
                            <label class="txtLabel">Section List</label>
                        </div>

                        <div class="box-body" >
                            <div class="cv-spinner" id="loadersection">
                                <span class="spinner"></span>
                            </div>
                            <table id="tblsectionlist" class="table table-striped table-bordered table-hover table-condensed" style="width: 100%">
                                <thead>
                                    <tr>
                                        <th>Gid</th>
                                        <th>Code	</th>
