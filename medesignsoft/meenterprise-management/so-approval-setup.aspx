<%@ Page Title="" Language="C#" MasterPageFile="~/medesignsoft.Master" AutoEventWireup="true" CodeBehind="so-approval-setup.aspx.cs" Inherits="medesignsoft.meenterprise_management.so_approval_setup" %>
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    <section class="content-header">
        ...
        <style>
            ...
            #tblsoapprovelist tbody tr:hover {
                color: red;
        ...
        <script>
            $(document).ready(function () {
                ...
                btnaddnew.click(function () {
                    window.location.href = "so-approval-edit.aspx?opt=optso&mod=new";
                });
                ...
                function reloaddata() {
                    $.ajax({
                        url: 'general-services.asmx/getSoApprovalList',
                        method: 'post',
                        //data: {
                        //    yearstart: yearstart,
                        //    yearend: yearend,
                        ...
                        beforeSend: function () {
                            $('#loader').show();
                            $("#tblsoapprovelist tr td").remove();

                        },
                        success: function (data) {
                            var table;
                            table = $('#tblsoapprovelist').DataTable();
                            table.clear();

                            if (data != '') {
                                $.each(data, function (i, item) {
                                    table.row.add([data[i].SoApproveID, data[i].imEmployeeGidU, data[i].UFullName, data[i].UPositionName
                                        , data[i].imEmployeeGidH, data[i].HFullName, data[i].HPositionName, data[i].Remark,
                            ...
                            $('#tblsoapprovelist td:nth-of-type(4)').addClass('column_hover');
                            $('#tblsoapprovelist td:nth-of-type(5)').addClass('column_hover');
                            ...
                            $('#tblsoapprovelist tbody').on('click', 'td', function (e) {
                                e.preventDefault();
                                ...
                                var gid = $("#tblsoapprovelist").find('tr:eq(' + rIndex + ')').find('td:eq(0)');

                                //console.log(gid.text());
                                if (rIndex != 0 & cIndex == 12) {
                                    window.location.href = "so-approval-edit.aspx?opt=optso&mod=edit&gid=" + gid.text();
                                }

                                if (rIndex != 0 & cIndex == 13) {
                                    window.location.href = "so-approval-edit.aspx?opt=optso&mod=del&gid=" + gid.text();
                                }
                            });
                        }
                    });
                ...
                <div class="">
                    <div class="box box-solid">
                        <div class="box-header with-border">
                            ...
                            <label class="txtLabel">รายการผู้อนุมัติการขายสินค้า</label>
                        </div>
                        ...
                        <div class="box-body">
                            ...
                            <table id="tblsoapprovelist" class="table table-striped table-bordered table-hover table-condensed" style="width: 100%">
                                <thead>
                                    <tr>
                                        <th>ID</th>
                                        <th>GidU</th>
