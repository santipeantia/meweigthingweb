<%@ Page Title="" Language="C#" MasterPageFile="~/medesignsoft.Master" AutoEventWireup="true" CodeBehind="ic-warehousezone-setup.aspx.cs" Inherits="medesignsoft.meenterprise_management.ic_warehousezone_setup" %>
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    <section class="content-header">
        ...
        <script>
            $(document).ready(function () {
                ...
                btnaddnew.click(function () {
                    window.location.href = "ic-warehousezone-edit.aspx?opt=optic&mod=new";
                });
                ...
                function reloaddata() {
                    $.ajax({
                        ...
                        beforeSend: function () {
                            $('#loader').show();
                            $("#tblwarehousezonelist tr td").remove();

                        },
                        success: function (data) {
                            var table;
                            table = $('#tblwarehousezonelist').DataTable();
                            table.clear();                          

                            if (data != '') {
                                $.each(data, function (i, item) {
                                    table.row.add([data[i].WhZoneID, data[i].WhID, data[i].WhDesc, data[i].WhZoneCode, data[i].WhZoneDesc
                            ...
                            $('#tblwarehousezonelist td:nth-of-type(6)').addClass('column_hover');
                            $('#tblwarehousezonelist td:nth-of-type(7)').addClass('column_hover');

                            $('#tblwarehousezonelist td').click(function () {
                                rIndex = this.parentElement.rowIndex;
                                cIndex = this.cellIndex;
                                console.log('row : ' + rIndex + ' cell: ' + cIndex)

                                if (rIndex != 0 & cIndex == 6) {
                                    var gid = $("#tblwarehousezonelist").find('tr:eq(' + rIndex + ')').find('td:eq(0)');
                                    ...
                                    window.location.href = "ic-warehousezone-edit.aspx?opt=optic&mod=edit&gid=" + gid.text();
                                }

                                if (rIndex != 0 & cIndex == 7) {
                                    var gid = $("#tblwarehousezonelist").find('tr:eq(' + rIndex + ')').find('td:eq(0)');
                                    console.log(gid.text());
                                    window.location.href = "ic-warehousezone-edit.aspx?opt=optic&mod=del&gid=" + gid.text();
                                }
        ...
        <h1>Warehouse Zone Setup <%--step 1 check pages content name--%>
                ...
                <div class="">
                    <div class="box box-solid">
                        ...
                        <div class="box-body">
                            ...
                            <table id="tblwarehousezonelist" class="table table-striped table-bordered table-hover table-condensed" style="width: 100%">
                                <thead>
                                    <tr>
                                        <th>ID</th>
                                        <th>WHID</th>
