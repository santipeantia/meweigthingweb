<%@ Page Title="" Language="C#" MasterPageFile="~/medesignsoft.Master" AutoEventWireup="true" CodeBehind="ic-goodunit-setup.aspx.cs" Inherits="medesignsoft.meenterprise_management.ic_goodunit_setup" %>
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    <section class="content-header">
        ...
        <style>
            ...
            #tblgoodunitlist tbody tr:hover {
                color: red;
        ...
        <script>
            $(document).ready(function () {
                ...
                btnaddnew.click(function () {
                    window.location.href = "ic-goodunit-edit.aspx?opt=optic&mod=new";
                });
                ...
                function reloaddata() {
                    $.ajax({
                        ...
                        beforeSend: function () {
                            $('#loader').show();
                            $("#tblgoodunitlist tr td").remove();

                        },
                        success: function (data) {
                            var table;
                            table = $('#tblgoodunitlist').DataTable();
                            table.clear();

                            if (data != '') {
                                $.each(data, function (i, item) {
                                    table.row.add([data[i].GoodUnitID, data[i].GoodUnitCode, data[i].GoodUnitDesc
                            ...
                            $('#tblgoodunitlist td:nth-of-type(4)').addClass('column_hover');
                            $('#tblgoodunitlist td:nth-of-type(5)').addClass('column_hover');

                            $('#tblgoodunitlist td').click(function () {
                                rIndex = this.parentElement.rowIndex;
                                cIndex = this.cellIndex;
                                console.log('row : ' + rIndex + ' cell: ' + cIndex)

                                if (rIndex != 0 & cIndex == 4) {
                                    var gid = $("#tblgoodunitlist").find('tr:eq(' + rIndex + ')').find('td:eq(0)');
                                    ...
                                    window.location.href = "ic-goodunit-edit.aspx?opt=optic&mod=edit&gid=" + gid.text();
                                }

                                if (rIndex != 0 & cIndex == 5) {
                                    var gid = $("#tblgoodunitlist").find('tr:eq(' + rIndex + ')').find('td:eq(0)');
                                    console.log(gid.text());
                                    window.location.href = "ic-goodunit-edit.aspx?opt=optic&mod=del&gid=" + gid.text();
                                }
        ...
        <h1>Good Unit Setup <%--step 1 check pages content name--%>
                ...
                <div class="">
                    <div class="box box-solid">
                        ...
                        <div class="box-body">
                            ...
                            <table id="tblgoodunitlist" class="table table-striped table-bordered table-hover table-condensed" style="width: 100%">
                                <thead>
                                    <tr>
                                        <th>ID</th>
                                        <th>CODE</th>
