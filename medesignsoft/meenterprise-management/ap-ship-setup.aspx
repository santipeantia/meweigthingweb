<%@ Page Title="" Language="C#" MasterPageFile="~/medesignsoft.Master" AutoEventWireup="true" CodeBehind="ap-ship-setup.aspx.cs" Inherits="medesignsoft.meenterprise_management.ap_ship_setup" %>
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    <section class="content-header">
        ...
        <style>
            ...
            #tblshipmentlist tbody tr:hover {
                color: red;
        ...
        <script>
            $(document).ready(function () {
                ...
                btnaddnew.click(function () {
                    window.location.href = "ap-ship-edit.aspx?opt=optap&mod=new";
                });
                ...
                function reloaddata() {
                    $.ajax({
                        url: 'general-services.asmx/getShipmentList',
                        method: 'post',
                        //data: {
                        //    yearstart: yearstart,
                        //    yearend: yearend,
                        ...
                        beforeSend: function () {
                            $('#loader').show();
                            $("#tblshipmentlist tr td").remove();

                        },
                        success: function (data) {
                            var table;
                            table = $('#tblshipmentlist').DataTable();
                            table.clear();

                            if (data != '') {
                                $.each(data, function (i, item) {
                                    table.row.add([data[i].ShipmentID, data[i].ShipmentCode, data[i].ShipmentName
                                        , data[i].ShipmentNameEng, data[i].Remark, data[i].edit, data[i].trash]);
                                });
                            }
                            table.draw();
                            $('#loader').hide();

                            $('#tblshipmentlist td:nth-of-type(5)').addClass('column_hover');
                            $('#tblshipmentlist td:nth-of-type(6)').addClass('column_hover');

                            $('#tblshipmentlist td').click(function () {
                                rIndex = this.parentElement.rowIndex;
                                cIndex = this.cellIndex;
                                console.log('row : ' + rIndex + ' cell: ' + cIndex)

                                if (rIndex != 0 & cIndex == 5) {
                                    var gid = $("#tblshipmentlist").find('tr:eq(' + rIndex + ')').find('td:eq(0)');
                                    ...
                                    window.location.href = "ap-ship-edit.aspx?opt=optap&mod=edit&gid=" + gid.text();
                                }

                                if (rIndex != 0 & cIndex == 6) {
                                    var gid = $("#tblshipmentlist").find('tr:eq(' + rIndex + ')').find('td:eq(0)');
                                    console.log(gid.text());
                                    window.location.href = "ap-ship-edit.aspx?opt=optap&mod=del&gid=" + gid.text();
                                }
        ...
        <h1>Shipment Setup <%--step 1 check pages content name--%>
                ...
                <div class="">
                    <div class="box box-solid">
                        <div class="box-header with-border">
                            ...
                            <label class="txtLabel">Shipment List</label>
                        </div>
                        ...
                        <div class="box-body">
                            ...
                            <table id="tblshipmentlist" class="table table-striped table-bordered table-hover table-condensed" style="width: 100%">
                                <thead>
                                    <tr>
                                        <th>ID</th>
                                        <th>CODE</th>
