using System;
using System.Collections.Generic;
using System.Linq;
using System.Web;

namespace medesignsoft
{
    public class cVendorsById
    {
        public string VendorID { get; set; }
        public string VendorGroupID { get; set; }
        public string VendorGroupName { get; set; }
        public string VendorCode { get; set; }
        public string VendorName { get; set; }
        public string VendorNameEng { get; set; }
        public string ShortName { get; set; }
        public string VendorStartDate { get; set; }
        public string Active { get; set; }
        public string activename { get; set; }
        public string InactiveDate { get; set; }
        public string adPaymentTypeID { get; set; }
        public string PaymentTypeDesc { get; set; }
        public string VendorTypeID { get; set; }
        public string VendorTypeName { get; set; }
        public string TaxId { get; set; }
        public string VendorAddr1 { get; set; }
        public string VendorAddr2 { get; set; }
        public string District { get; set; }
        public string Amphur { get; set; }
        public string adProvinceID { get; set; }
        public string ProvinceName { get; set; }
        public string PostCode { get; set; }
        public string ContAddr1 { get; set; }
        public string ContAddr2 { get; set; }
        public string ContDistrict { get; set; }
        public string ContAmphur { get; set; }
        public string ContProvince { get; set; }
        public string ConProvinceName { get; set; }
        public string ContPostCode { get; set; }
        public string ContHomePage { get; set; }
        public string ContTel { get; set; }
        public string ContFax { get; set; }
        public string CardNo { get; set; }
        public string VATGroupID { get; set; }
        public string VATGroupDesc { get; set; }
        public string imBranchID { get; set; }
        public string Birthdate { get; set; }
        public string ContTelExtend1 { get; set; }
        public string ContTelExtend2 { get; set; }
        public string BranchCode { get; set; }
        public string BranchName { get; set; }
        public string CreditDays { get; set; }
        public string CreditAmnt { get; set; }
        public string CreatedBy { get; set; }
        public string CreatedDate { get; set; }
        public string UpdatedBy { get; set; }
        public string UpdateDate { get; set; }
        public string SalesCode { get; set; }
    }
}