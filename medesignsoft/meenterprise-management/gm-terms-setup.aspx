<%@ Page Title="" Language="C#" MasterPageFile="~/medesignsoft.Master" AutoEventWireup="true" CodeBehind="gm-terms-setup.aspx.cs" Inherits="medesignsoft.meenterprise_management.gm_terms_setup" %>
        ...
        <script>
            $(document).ready(function () {
                ...
                btnaddnew.click(function () {
                    window.location.href = "gm-terms-edit.aspx?opt=optgen&mod=new";
                });
                ...
                function reloaddata() {
                    $.ajax({
                        ...
                        beforeSend: function () {
                            $('#loader').show();
                            $("#tbltermslist tr td").remove();

                        },
                        success: function (data) {
                            var table;
                            table = $('#tbltermslist').DataTable();
                            table.clear();

                            if (data != '') {
                                $.each(data, function (i, item) {
                                    table.row.add([data[i].adPaymentTypeID, data[i].PaymentTypeDesc, data[i].PaymentTypeDesc2
                            ...
                            $('#tbltermslist td:nth-of-type(6)').addClass('column_hover');
                            $('#tbltermslist td:nth-of-type(8)').addClass('column_hover');

                            $('#tbltermslist td').click(function () {
                                rIndex = this.parentElement.rowIndex;
                                cIndex = this.cellIndex;
                                console.log('row : ' + rIndex + ' cell: ' + cIndex)

                                if (rIndex != 0 & cIndex == 7) {
                                    var gid = $("#tbltermslist").find('tr:eq(' + rIndex + ')').find('td:eq(0)');
                                    console.log(gid.text());
                                    window.location.href = "gm-terms-edit.aspx?opt=optgen&mod=edit&gid=" + gid.text();
                                }

                                if (rIndex != 0 & cIndex == 8) {
                                    var gid = $("#tbltermslist").find('tr:eq(' + rIndex + ')').find('td:eq(0)');
                                    console.log(gid.text());
                                    window.location.href = "gm-terms-edit.aspx?opt=optgen&mod=del&gid=" + gid.text();
                                }
                ...
                <div class="">
                    <div class="box box-solid">
                        <div class="box-header with-border">
                            ...
                            <label class="txtLabel">Terms Payment List</label>
                        </div>
                        ...
                        <div class="box-body">
                            ...
                            <table id="tbltermslist" class="table table-striped table-bordered table-hover table-condensed" style="width: 100%">
                                <thead>
                                    <tr>
