<%@ Page Title="" Language="C#" MasterPageFile="~/medesignsoft.Master" AutoEventWireup="true" CodeBehind="so-approvelevel-setup.aspx.cs" Inherits="medesignsoft.meenterprise_management.so_approvelevel_setup" %>
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    <section class="content-header">
        ...
        <style>
            ...
            #tblsoapprovelevellist tbody tr:hover {
                color: red;
        ...
        <script>
            $(document).ready(function () {
                ...
                btnaddnew.click(function () {
                    window.location.href = "so-approvelevel-edit.aspx?opt=optso&mod=new";
                });
                ...
                function reloaddata() {
                    $.ajax({
                        url: 'general-services.asmx/getSoApprovalLevelList',
                        method: 'post',
                        //data: {
                        //    yearstart: yearstart,
                        //    yearend: yearend,
                        ...
                        beforeSend: function () {
                            $('#loader').show();
                            $("#tblsoapprovelevellist tr td").remove();

                        },
                        success: function (data) {
                            var table;
                            table = $('#tblsoapprovelevellist').DataTable();
                            table.clear();

                            if (data != '') {
                                $.each(data, function (i, item) {
                                    table.row.add([data[i].SoAppLevelID, data[i].imEmployeeGid, data[i].FullName, data[i].SoLevelID, data[i].SoLevelCode, data[i].SoLevelDesc,
                                    data[i].PositionName, data[i].Remark, data[i].UserCreate, data[i].CreateDate, data[i].UserUpdate, data[i].LasteDate, data[i].edit, data[i].trash]);
                                });
                            }
                            table.draw();
                            $('#loader').hide();

                            $('#tblsoapprovelevellist td:nth-of-type(4)').addClass('column_hover');
                            $('#tblsoapprovelevellist td:nth-of-type(5)').addClass('column_hover');

                            //$('#tblsoapprovelist td').click(function () {
                            ...
                            $('#tblsoapprovelevellist tbody').on('click', 'td', function (e) {
                                e.preventDefault();
                                ...
                                var gid = $("#tblsoapprovelevellist").find('tr:eq(' + rIndex + ')').find('td:eq(0)');

                                //console.log(gid.text());
                                if (rIndex != 0 & cIndex == 12) {
                                    window.location.href = "so-approvelevel-edit.aspx?opt=optso&mod=edit&gid=" + gid.text();
                                }

                                if (rIndex != 0 & cIndex == 13) {
                                    window.location.href = "so-approvelevel-edit.aspx?opt=optso&mod=del&gid=" + gid.text();
                                }
                            });
                        }
                    });
        ...
        <h1>SO Approval Setup <%--step 1 check pages content name--%>
                ...
                <div class="">
                    <div class="box box-solid">
                        <div class="box-header with-border">
                            ...
                            <label class="txtLabel">ลำดับผู้อนุมัติการขาย</label>
                        </div>
                        ...
                        <div class="box-body">
                            ...
                            <table id="tblsoapprovelevellist" class="table table-striped table-bordered table-hover table-condensed" style="width: 100%">
                                <thead>
                                    <tr>
                                        <th>ID</th>
                                        <th>EmpGid</th>
